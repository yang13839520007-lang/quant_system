# -*- coding: utf-8 -*-
"""
Created on Thu Mar 19 11:43:19 2026
...
TARGET_FILE = PROJECT_ROOT / "generate_market_signal_snapshot.py"
MODULE_ALIAS = "_quant_system_root_generate_market_signal_snapshot"
...
def normalize_code(*args, **kwargs):
    return getattr(_load_root_module(), "normalize_code")(*args, **kwargs)


def pick_first_column(*args, **kwargs):
    return getattr(_load_root_module(), "pick_first_column")(*args, **kwargs)


def resolve_project_root(*args, **kwargs):
    return getattr(_load_root_module(), "resolve_project_root")(*args, **kwargs)


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="行情快照补数脚本入口")
...
def _resolve_runtime_kwargs(**kwargs: Any) -> dict[str, Any]:
    ...
    candidate_path = (
        kwargs.get("candidate_path")
        or kwargs.get("candidates_path")
    ...
    for param in params:
        ...
        if param.name in runtime_kwargs and runtime_kwargs[param.name] is not None:
            ...
            if "date" in name:
                ...
                provided = value is not None
            elif any(token in name for token in ["candidate", "input_file", "code_file"]):
                value = runtime_kwargs.get("candidate_path")
                provided = value is not None
...
def main(**kwargs: Any):
    ...
    for entry_name in ("main", "run", "generate_market_signal_snapshot"):
        func = getattr(module, entry_name, None)
        if callable(func):
            return _invoke_callable(func, runtime_kwargs)

    raise AttributeError(f"{TARGET_FILE.name} 未找到可调用入口: main/run/generate_market_signal_snapshot")
...
def generate_market_signal_snapshot(**kwargs: Any):
    return main(**kwargs)
